        ...
        for node in tqdm(data.findall(".//sentence"), unit="sentence"):
            ...
            for opinion in node.findall(".//Opinion"):
                ...
                if polarity not in labels:
                    raise ValueError(
                        f'Unknown polarity "{polarity}" found at sentence "{sentence}"'
                    )
...
def main():
    ...
    parser.add_argument(
        "--phase", default="Train", help="The phase of the dataset (Train or Test)"
    )
    parser.add_argument(
        "--ont-hops",
        default=None,
    ...
    if not generate_all:
        ...
        embeddings_dir = EmbeddingsDataset(
            year=year,
            device=device,
            phase=phase,
            ont_hops=ont_hops,
    ...
    for phase in ["Train", "Test"]:
        ...
        embeddings_dir = EmbeddingsDataset(
            year=year, device=device, phase=phase, ont_hops=None, empty_ok=True
        ).dir
        generate_embeddings(embeddings_layer, data, embeddings_dir)
