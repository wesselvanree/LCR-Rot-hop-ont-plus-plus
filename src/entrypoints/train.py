...
def main():
    ...
    train_dataset = EmbeddingsDataset(
        year=year, device=device, phase="Train", ont_hops=ont_hops
    )
    ...
    if val_ont_hops is not None:
        train_val_dataset = EmbeddingsDataset(
            year=year, device=device, phase="Train", ont_hops=val_ont_hops
        )
    ...
    training_loader = DataLoader(
        training_subset, batch_size=batch_size, collate_fn=lambda batch: batch
    )
