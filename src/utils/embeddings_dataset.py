...
class EmbeddingsDataset(Dataset):
    ...
    def __getitem__(self, item: int):
        ...
        label: torch.Tensor = torch.tensor(
            data["label"], requires_grad=False, device=self.device
        )
...
def train_validation_split(
    dataset: EmbeddingsDataset, validation_size=0.2, seed: Optional[float] = None
):
